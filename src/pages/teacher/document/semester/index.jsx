...
const Semester = () => {
  ...
  const onFinish = (values) => {
    // Chuyển đổi mảng các môn học đã chọn thành chuỗi có dấu phẩy và cách
    const selectedSubjects = values.subject.join(', ');
    const selectedClasses = values.class.join(', '); // Kết hợp các lớp đã chọn thành chuỗi
  
    if (editMode) {
      // Cập nhật tài liệu
      const updatedDocuments = documents.map(doc =>
        doc.id === currentDocument.id ? { ...doc, ...values, subject: selectedSubjects, class: selectedClasses, date: values.testDate.format('YYYY-MM-DD') } : doc
      );
      setDocuments(updatedDocuments);
      notification.success({ message: 'Tài liệu đã được cập nhật thành công!' });
    } else {
      // Thêm tài liệu mới
      const newDocument = { ...values, subject: selectedSubjects, class: selectedClasses, id: documents.length + 1, date: values.testDate.format('YYYY-MM-DD') };
  ...
  return (
    <div className="h-screen p-4 overflow-auto">
      <h2 className="text-2xl font-semibold">Nội dung Học kỳ</h2>
      <p>
        Đây là nội dung cho việc Ôn tập thi Học kỳ. Bạn có thể thêm thông tin, hướng dẫn,
        và tài liệu liên quan đến việc Ôn tập, Kiểm tra ở đây.
      </p>
      {/* Nút Thêm Tài liệu */}
      ...
      {/* Modal */}
      <Modal
        title={editMode ? "Cập nhật Tài liệu Học kỳ" : "Thêm Tài liệu Học kỳ"}
        visible={visible}
        onCancel={handleCancel}
        footer={null}
      >
        <Form
          name="add_semester_document"
          onFinish={onFinish}
          layout="vertical"
      ...
      {/* Danh sách tài liệu */}
      <h3 className="my-3 text-xl font-semibold">Danh sách Tài liệu - Lịch Học kỳ</h3>
      <List
        bordered
        ...
        renderItem={item => (
            <List.Item className="flex flex-wrap items-center justify-between">
                <div className="flex-1 mr-4">
                    <strong>Môn:</strong> {item.subject} |
                    <strong> Học kỳ:</strong> {item.semester} |
